 ...
 */
#ifdef DEBUG
#define LOADER_BIN_SIZE    0xC8000 /* 800k */
#else
#define LOADER_BIN_SIZE    0x64000 /* 400k */
#endif
...
/* startap size 12KB (must be 4KB aligned), otherwise, need to
 *  use paged_buffer_t to force page alignment
 */
#define STARTAP_IMG_SIZE                0x3000

/* xmon_loader image size running in RAM, 60KB */
#define XMON_LOADER_IMG_SIZE            0xF000
 ...
 */
#ifdef DEBUG
#define XMON_DEFAULT_TOTAL_SIZE  0x900000 /* include xmon img/stack/heap */
#else
#define XMON_DEFAULT_TOTAL_SIZE  0x600000
#endif
...
typedef struct {
	uint32_t img_base;
	uint32_t total_size;       /* must be >= hdr_info.load_size */
...
typedef struct {
	uint32_t eax;   /* MB magic */
	uint32_t ebx;   /* MB info passed on from loader */
	uint32_t ecx;
	uint32_t edx;
	...
	uint32_t eip;
	uint32_t eflags;
} multiboot_initial_state_t;


typedef struct {
	/* save initial mb boot cpu state when bootstub passed on to us */
	multiboot_initial_state_t mb_initial_state;


	/* starter fills these below */
	uint32_t loader_mem_addr;
	uint32_t runtime_mem_addr;
	...
	/* modules loaded by grub, actually we can get it from ebx above*/
	/*multiboot_module_t *grub_module;*/
} xmon_desc_t;

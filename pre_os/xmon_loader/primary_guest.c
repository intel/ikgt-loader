...
mon_guest_startup_t *setup_primary_guest_env(xmon_desc_t *xd)
{
	 ...
	 */
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RIP] =
		(uint64_t)((uint32_t)&primary_guest_entry);

	primary_guest_bsp_cpu->gp.reg[IA32_REG_RSP] = xd->mb_initial_state.esp;         /* use starter stack */
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RBX] = xd->mb_initial_state.ebx;         /* boot info */
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RAX] = xd->mb_initial_state.eax;         /* boot magic */
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RFLAGS] = xd->mb_initial_state.eflags;

	/* use ecx to pass on xmon_desc_t *xd */
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RCX] = (uint64_t)((uint32_t)xd);

	primary_guest_bsp_cpu->gp.reg[IA32_REG_RDX] = xd->mb_initial_state.edx;
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RBP] = xd->mb_initial_state.ebp;
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RSI] = xd->mb_initial_state.esi;
	primary_guest_bsp_cpu->gp.reg[IA32_REG_RDI] = xd->mb_initial_state.edi;
	...
	/* construct primary guest state */
	{
		...
		primary_guest->cpu_states_array =
			(uint64_t)((uint32_t)primary_guest_bsp_cpu) &
			0x0000000FFFFFFFF;

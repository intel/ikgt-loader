...
void install_exception_handler(uint32_t exception_index, uint32_t handler_addr)
{
...
void setup_idt()
{
	...
	install_exception_handler
		(IA32_EXCEPTION_VECTOR_RESERVED_SIMD_FLOATING_POINT_NUMEIC_ERROR,
		(uint32_t)exception_handler_reserved_simd_floating_point_numeric_error);

	idt_descriptor.base = (uint32_t)(xmon_idt);
	idt_descriptor.limit = sizeof(ia32_idt_gate_descriptor_t) * 32 - 1;

	p_idt_descriptor = (uint32_t)&idt_descriptor;
	__asm__ __volatile__ (
		"mov   %0, %%edx\n"
		"lidt  (%%edx)\n"
		:
